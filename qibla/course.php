...
class Course extends Canvas{
  ...
  public function enroll_teacher($course_id,$user_id, $type = "TeacherEnrollment", $enrollment_state = "active",  $notify = 0){
    $data = json_encode(array("enrollment" => array( "user_id" => $user_id, "type" => $type, "enrollment_state" => $enrollment_state, "notify" => $notify)));
    return $this->post_json("/courses/".$course_id."/enrollments",$data);
  } 
	public function conclude_enrollment($course_id,$enrollment_id){
  	return $this->delete_json("/courses/".$course_id."/enrollments/".$enrollment_id."?task=conclude");
	}

  public function alumni_user($course_id,$user_id, $type = "ObserverEnrollment", $enrollment_state = "active",  $notify = 0){
    $data = json_encode(array("enrollment" => array( "user_id" => $user_id,"role" => "Alumni", "type" => $type, "enrollment_state" => $enrollment_state, "notify" => $notify)));
    return $this->post_json("/courses/".$course_id."/enrollments",$data);
  } 
