...
class WC_Checkout {
	...
	public function __construct () {
		...
		$this->checkout_fields['account']['billing_email'] = array(
				'type' 			=> 'text',
				'label' 		=> __( 'User Email', 'woocommerce' ),
				'placeholder' 	=> _x( 'Email', 'placeholder', 'woocommerce' ),
				'class' 			=> array( 'form-row' )
			);
		$this->checkout_fields['account']['account_password'] = array(
			'type' 				=> 'password',
			'label' 			=> __( 'User password', 'woocommerce' ),
	...
	public function process_checkout() {
		...
		if ( $this->creating_account ) {
			...
			if ( empty( $this->posted['billing_email'] ) )
					$woocommerce->add_error( __( 'Please enter an user email.', 'woocommerce' ) );
			// Validate passwords
