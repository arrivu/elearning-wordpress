			...
			?><p>
				<?php
					...
					if ( in_array( $the_order->status, array( 'pending', 'on-hold', 'processing' ) ) )
						//$order_com = new WC_Order($post->ID );
						//print_r($post);
						//$items = $order_com->get_items();
						
						$actions['complete'] = array(
							'url' 		=> wp_nonce_url( admin_url( 'admin-ajax.php?action=woocommerce-mark-order-complete&order_id=' . $post->ID.'&user_id='.$the_order->user_id ), 'woocommerce-mark-order-complete' ),
