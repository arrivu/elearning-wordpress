...
function woocommerce_product_data_box() {
	...
	if ( $terms = wp_get_object_terms( $post->ID, 'product_type' ) )
		...
		$product_type = 'simple';
	if ( $terms = wp_get_object_terms( $post->ID, 'instructor_type' ) )
		$instructor_type = sanitize_title( current( $terms )->name );
	else
		$instructor_type = '';
...
$cat_idm = 15; // The category id to Multimedia
$sqlMulti = "SELECT p.*,t.* FROM $wpdb->posts p
JOIN $wpdb->term_relationships tr ON (p.ID = tr.object_id)
JOIN $wpdb->term_taxonomy tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id)
JOIN $wpdb->terms t ON (tt.term_id = t.term_id)
WHERE p.post_type='post'
AND p.post_status = 'publish'
AND tt.taxonomy = 'category'
AND t.term_id = $cat_idm
ORDER BY ID DESC";
$mypagesMulti = $wpdb->get_results($sqlMulti);

$cat_idins = 21; // The category id to Instructors
$sqlInst = "SELECT p.*,t.* FROM $wpdb->posts p
JOIN $wpdb->term_relationships tr ON (p.ID = tr.object_id)
JOIN $wpdb->term_taxonomy tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id)
JOIN $wpdb->terms t ON (tt.term_id = t.term_id)
WHERE p.post_type='post'
AND p.post_status = 'publish'
AND tt.taxonomy = 'category'
AND t.term_id = $cat_idins
ORDER BY ID DESC";
$mypagesInst = $wpdb->get_results($sqlInst);
//print_r($mypages);

$type_box1  = '<label for="product-type"><select id="instructor-type" name="instructor_type"><optgroup label="' . __( 'Photography', 'woocommerce' ) . '">';
		$type_box1 .='<option value=""></option>';	
		foreach ( $mypages as $value ){
		//if($value->ID)
				
		$type_box1 .= '<option value="' . esc_attr( $value->ID ) . '" ' . selected( $instructor_type, $value->ID, false ) .'>' . esc_html( $value->post_name ) . '</option>';
		
		}
		$type_box1 .= '</optgroup>';
		$type_box1 .='<optgroup label="' . __( 'Multimedia', 'woocommerce' ) . '">';
		foreach ( $mypagesMulti as $valuem ){
		//if($value->ID)
					
		$type_box1 .= '<option value="' . esc_attr( $valuem->ID ) . '" ' . selected( $instructor_type, $valuem->ID, false ) .'>' . esc_html( $valuem->post_name ) . '</option>';
						
		}
		$type_box1 .= '</optgroup>';
		$type_box1 .='<optgroup label="' . __( 'Instructors', 'woocommerce' ) . '">';
		foreach ( $mypagesInst as $valuein ){
		//if($value->ID)
					
		$type_box1 .= '<option value="' . esc_attr( $valuein->ID ) . '" ' . selected( $instructor_type, $valuein->ID, false ) .'>' . esc_html( $valuein->post_name ). '</option>';
		}
		$type_box1 .= '</optgroup>';
