    ...
    try {
      ...
      if (!is_user_logged_in()) {
        ...
        if ($ID) { // Login
          ...
          global $wpdb;
          $get_user_password="select * from wp_users where ID=".$ID;
          $get_password=$wpdb->get_row($get_user_password);
          //echo $get_password->user_email;
          $direct=ABSPATH;                 
          $config = parse_ini_file($direct."/config.ini");
          $casurl = $config["casurl"];
          $cookie_set_url = $config["cookieurl"];
          $data = array('username'=> $get_password->user_email,'password'=> $get_password->user_pass);
